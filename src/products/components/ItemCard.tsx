...
export const ItemCard = ({ product, quantity }: Props) => {
  ...
  function onAddToCart() {
    ...
    actionsProduct.addProductToCart(product.id)
    router.refresh();
  }
  ...
  function onRemoveItem() {
    ...
    actionsProduct.removeSingleItemFromCart(product.id)
    router.refresh();
  }
      ...
      <div className="px-5 pb-5 w-full flex flex-col mt-2">
        <a href="#">
          <h3 className="font-semibold text-xl tracking-tight text-white">
            { product.name } - <small className="text-sm">${ product.price.toFixed(3) }</small>
          </h3>
        </a>      
        ...
        <div className="flex flex-col items-start justify-between">
          ...
          <span className="font-bold text-white">
            Total: ${ (product.price * quantity).toFixed(3) }
          </span>
