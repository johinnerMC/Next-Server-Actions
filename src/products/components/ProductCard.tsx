...
import * as actionsProduct from "@/shopping-cart/actions/actions"
import { useRouter } from "next/navigation"

export const ProductCard = ({image, name, price, rating, id}: Product) => {

    const router = useRouter();

    const onAddToCart = () => {
        actionsProduct.addProductToCart(id);
        router.refresh();
    }
    const removeFromCart = () => {
        actionsProduct.removeProductFromCart(id);
        router.refresh();
    }
  return (
    <div className="bg-white shadow rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-100">
      ...
      <div className="px-5 pb-5">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xl font-bold text-gray-900 dark:text-white">${price.toFixed(3)}</span>
